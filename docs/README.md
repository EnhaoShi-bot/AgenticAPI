...
| v0.3.0 | 2026-08-16 | 核心中转链路上线：OpenAI 兼容 `/v1/chat/completions`（API 密钥鉴权、分组权限、余额计费、渠道轮询、流式透传）；用户表扩展管理员/余额/分组/状态等字段；新增 `api_key`、`logs` 表；控制台新增秘钥管理页与用户管理页（管理员）；登录注册访客、密钥与管理员操作全部接入日志。详见《[实现汇报-核心中转链路](实现汇报-核心中转链路.md)》                                                                                                                                                                                                                                                                                  |
