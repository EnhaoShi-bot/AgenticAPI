...
from sqlalchemy import DateTime, Integer, Float, String, Text, DECIMAL, UniqueConstraint, BigInteger, text
...
class ModelsTable(BaseModel):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True,
                                    comment="主键ID")  # 【修正】数据库为 bigint，和实际对齐
    ...
    is_request_mode: Mapped[bool] = mapped_column(Integer, nullable=False, default=0,
                                                  comment="是否按次计费模式 0否 1是")  # 【修正】SQLAlchemy 用 Integer 映射 tinyint(1)，但类型标注用 bool 更语义化
    per_request_price: Mapped[Decimal] = mapped_column(DECIMAL(12, 4), nullable=False, default=0,
                                                       comment="单次请求价格（按次计费生效）")
    ...
    is_pin: Mapped[bool] = mapped_column(Integer, nullable=False, default=0,
                                         comment="是否置顶 0否 1是")  # 【修正】同上，类型标注改为 bool
    is_log: Mapped[bool] = mapped_column(Integer, nullable=False, default=0,
                                         comment="是否开启日志记录 0否 1是")  # 【修正】同上
    ...
    support_vision: Mapped[bool] = mapped_column(Integer, nullable=False, default=0,
                                                 comment="是否支持视觉识图：0不支持 1支持")  # 【修正】同上
    icon: Mapped[str] = mapped_column(String(255), nullable=False, default="", comment="模型图标")
...
class ChannelsTable(BaseModel):
    ...
    support_models: Mapped[str | None] = mapped_column(Text, default=None,
                                                       comment='支持的模型列表，JSON数组["model-name"]')
    ...
    used_ratio: Mapped[float] = mapped_column(Float, nullable=False, default=0, comment="使用率(百分数)")
    description: Mapped[str | None] = mapped_column(Text, default=None, comment="渠道备注说明")

## ==================================================
# 当需要新增表时，只需在 mysql_tables.py 中定义新类并继承 BaseModel 即可
# 在FastAPI启动时，会自动扫描 mysql_tables.py 中所有继承 BaseModel 的表定义
